...
class RegisterCustomerForm(FlaskForm):
    ...
    last_name = StringField('Last name', validators=[DataRequired(), Length(min=3, max=255)])
    email = StringField('Email address', validators=[DataRequired(), Email()])
    password = PasswordField('Password', validators=[DataRequired(), Length(min=8, max=255)])
    password_confirm = PasswordField('Password Confirm', validators=[DataRequired(), EqualTo('password', message="Passwords must match.")])
